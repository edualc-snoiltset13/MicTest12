...
# Extensions accepted by the Anthropic Messages API for base64 image input.
...
try:
    ...
    # urlopen raises before we can read the body; pull it out for a useful message.
    sys.exit(f"API error {e.code}: {e.read().decode('utf-8', 'replace')}")

# Response.content is a list of typed blocks; tool/thinking blocks may appear alongside text.
